...
addov <- function(name, gr, col, label_col) {
  ...
  overlays[[length(overlays) + 1]] <<- list(name = name, gr = gr, col = col, label_col = label_col)
}
...
pick_name <- function(explicit, inline, default) {
  if (!identical(explicit, default)) return(explicit)
...
plot_kp <- function() {
  ...
  for (j in seq_len(n_overlay)) {
    ...
    if (length(ov$gr) > 0) {
      kpSegments(kp, chr = as.character(seqnames(ov$gr)),
